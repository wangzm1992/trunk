...
void DmpNudSensitiveDetector::Initialize(G4HCofThisEvent*){
 // NUDHitCollection = new DmpSimuNudHitsCollection
  ...
  NUDHitCollection0 = new DmpSimuNudHitsCollection(SensitiveDetectorName,collectionName[0]);
  NUDHitCollection1 = new DmpSimuNudHitsCollection(SensitiveDetectorName,collectionName[1]);
  NUDHitCollection2 = new DmpSimuNudHitsCollection(SensitiveDetectorName,collectionName[2]);
  NUDHitCollection3 = new DmpSimuNudHitsCollection(SensitiveDetectorName,collectionName[3]);
/* 
...
G4bool DmpNudSensitiveDetector::ProcessHits(G4Step* aStep, G4TouchableHistory*){
  ...
  for (G4int i=0;i<NbOfNUDTimeIntervals;i++) {
    ...
    if (TimeDown<StepTime && StepTime<=TimeUp) {
    // This TouchableHistory is used to obtain the physical volume of the hit
     ...
     // G4int StripNumber=acd_tile->GetCopyNo();
      ...
      if (NUDName == "NUD0"){
        if (HitID0[i]==-1){   // This is a new hit
  	    ...
  	    HitID0[i] = NUDHitCollection0->insert(PSSDHit) -1;
  	  }else{ // This is not new
  	    (*NUDHitCollection0)[HitID0[i]]->AddEnergy(EdepTime[i]);
  	  }
        ...
        if (HitID1[i]==-1){
  	    ...
  	    PSSDHit->SetPos(aStep->GetPreStepPoint()->GetPosition());
          PSSDHit->SetTime((i+1)*100.);   
  	    HitID1[i] = NUDHitCollection1->insert(PSSDHit) -1;
  	  }else{ // This is not new
  	    (*NUDHitCollection1)[HitID1[i]]->AddEnergy(EdepTime[i]);
  	  }
        ...
        if (HitID2[i]==-1){
  	    ...
  	    PSSDHit->SetPos(aStep->GetPreStepPoint()->GetPosition());
          PSSDHit->SetTime((i+1)*100.);   
  	    HitID2[i] = NUDHitCollection2->insert(PSSDHit) -1;
  	  }else{ // This is not new
  	    (*NUDHitCollection2)[HitID2[i]]->AddEnergy(EdepTime[i]);
  	  }
        ...
        if (HitID3[i]==-1){
  	    ...
  	    PSSDHit->SetPos(aStep->GetPreStepPoint()->GetPosition());
          PSSDHit->SetTime((i+1)*100.);   
  	    HitID3[i] = NUDHitCollection3->insert(PSSDHit) -1;
  	  }else{ // This is not new
  	    (*NUDHitCollection3)[HitID3[i]]->AddEnergy(EdepTime[i]);
  	  }
...
static G4int HCID3 = -1;
  ...
  if(HCID0<0)   HCID0 = G4SDManager::GetSDMpointer()->GetCollectionID(collectionName[0]);
  if(HCID1<0)   HCID1 = G4SDManager::GetSDMpointer()->GetCollectionID(collectionName[1]);
  if(HCID2<0)   HCID2 = G4SDManager::GetSDMpointer()->GetCollectionID(collectionName[2]);
  if(HCID3<0)   HCID3 = G4SDManager::GetSDMpointer()->GetCollectionID(collectionName[3]);
