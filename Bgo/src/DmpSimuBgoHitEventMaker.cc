...
#include "TTree.h"
#include "DmpDetectorBgo.h"    // in Kernel
using namespace DmpDetector::Bgo;  // use parameter kPlaneNo
...
void DmpSimuBgoHitEventMaker::AddCollections(const G4Run* aRun)
{ 
  //fDataMgr = (DmpSimSvcDataMgr*)gCore->ServiceManager()->Get("Sim/DataMgr");
  //fBgoHits  = fDataMgr->NewOutCollection(const_cast<char *>("DmpSimuBgoHitsCollection"),  const_cast<char *>("DmpBgoHit"));
  fBgoHits = new TClonesArray("DmpBgoHit",10000);
  DmpIOSvc::GetInstance()->AddOutputContainer("DmpSimuBgoHitsCollection",fBgoHits);
}
...
void DmpSimuBgoHitEventMaker::BeginRun(const G4Run* run)
{
}
...
/* needless
    ...
    if(kPlaneNo != (dmpDetector->GetBgoDetectorDescription()->GetNoOfCALLayers())/2) G4cout<< "WARNING CALLayers is not "<< kPlaneNo<< "!!!"<< G4endl;
*/
// *TODO:  7 = kPlaneNo
...
*/
  ...
  for (int i=0;i<n_hit;i++) {
    ...
    if(Edep<0.0)  {DmpLogDebug<<"E of hit < 0.0"<<G4endl;continue;}
    ...
    { // for single G4Step ??
      ...
      // why in Edep > 0.0 ?? 
    DmpBgoHit *hit = (DmpBgoHit*)fBgoHits->ConstructedAt(fBgoHits->GetLast()+1);
    hit->Hit_e    = Edep;
...
*/
    ...
    /*
    // *
    // * TODO: this should be moved to the analysis/reconstruction package!
    // *
    //--------------------------------------------------------------------
    ...
    if(CALType==1) { // x layers
      ...
      DmpLogWarning << "WARNING wrong CALType!!! " << CALType << G4endl;
    }
    //--------------------------------------------------------------------
    */
