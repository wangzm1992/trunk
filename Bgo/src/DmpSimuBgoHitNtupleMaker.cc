...
{  
  ...
  if (CHC) {
    ...
    if(eventNumber == debugEvent || debugEvent == -1) {
      DmpLogDebug << "Number of calorimeter hits in this event =  " << n_hit << G4endl;
      DmpLogDebug << "  edp  " << "  bar " << " plane " << " type "
	     << "  x    " << "   y    " << "     z    " <<  G4endl;       
    }   
    ...
    if(NbOfCALLayers != (dmpDetector->GetBgoDetectorDescription()->GetNbOfCALLayers())/2) DmpLogDebug<< "WARNING NbOfCALLayers is not "<< NbOfCALLayers<< "!!!"<< G4endl;
	...
	++tt_CalHit_n;
      ...
      if(eventNumber == debugEvent || debugEvent == -1) {
        DmpLogDebug << Edep/MeV <<  "   "  << CALBarNumber << "    " << CALPlaneNumber <<  "    " << CALType
	...
	++tt_CalLayer_nlayer_1MeV;
      ...
      if(eventNumber == debugEvent || debugEvent == -1) {
        DmpLogDebug << " xLayer " << i << " " << " esum " << xLayerSum[i] << " esumW " << xLayerSumW[i] << G4endl;
        DmpLogDebug << " yLayer " << i << " " << " esum " << yLayerSum[i] << " esumW " << yLayerSumW[i] << G4endl;
        DmpLogDebug << " 1MeV xLayer " << i << " " << " esum " << xLayerSum_1MeV[i] << " esumW " << xLayerSumW_1MeV[i] << G4endl;
        DmpLogDebug << " 1MeV yLayer " << i << " " << " esum " << yLayerSum_1MeV[i] << " esumW " << yLayerSumW_1MeV[i] << G4endl;
      }
    }

    if(eventNumber == debugEvent || debugEvent == -1) {
      DmpLogDebug << " total energy deposited: " << tt_cal_totE << " " << tt_cal_totE_1MeV << " "
