...
{ 
  ...
  */
  ...
  SetRndmSeedCmd = new G4UIcmdWithAnInteger("/random/setSeed",this);
  SetRndmSeedCmd->SetGuidance("Set random generator seed.");
  SetRndmSeedCmd->SetGuidance("See: http://proj-clhep.web.cern.ch/proj-clhep/manual/UserGuide/Random/Random.html");
  SetRndmSeedCmd->SetDefaultValue((G4int)12345);
...
{ 
  ...
  if( command == SetRndmSeedCmd)
    {dmpSimuPrimaryGeneratorAction->SetRandomSeed(SetRndmSeedCmd->GetNewIntValue(newValue));}
