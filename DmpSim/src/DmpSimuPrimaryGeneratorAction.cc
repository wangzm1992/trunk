...
DmpSimuPrimaryGeneratorAction::DmpSimuPrimaryGeneratorAction()
  :rndmFlag("off"),nSourceType(0),nSpectrumType(0),useGenFilter(false),passGenFilter(true),
   filterShadowed(false),filterUpward(false),filterBelowBGO(false),nFilterRejected(0),nGenerated(0),dVertexRadius(-1)
{
  ...
  DmpLogDebug << "PayLoadSize x, y, z  XY:" << PayLoadSizeXY << " Z:" <<PayLoadSizeZ << DmpLogEndl;
  DmpLogDebug << "Top of BGO at z = " << BgoTopZ << DmpLogEndl;
  G4ThreeVector dir0; 
...
DmpSimuPrimaryGeneratorAction::~DmpSimuPrimaryGeneratorAction()
{
  DmpLogInfo << "[DmpSimuPrimaryGeneratorAction::~DmpSimuPrimaryGeneratorAction] DmpSimuPrimaryGeneratorAction: number of event generated = " << nGenerated      << DmpLogEndl;
  DmpLogInfo << "[DmpSimuPrimaryGeneratorAction::~DmpSimuPrimaryGeneratorAction]   number of event rejected by filter = " << nFilterRejected << DmpLogEndl;
...
void DmpSimuPrimaryGeneratorAction::SetupSourceType(G4int val)
{
  ...
  G4double PayLoadSizeZ = (dmpDetector->GetPayloadSizeZ());
  G4double PayLoadSizeXY = (dmpDetector->GetPayloadSizeXY());
  DmpLogDebug << "[DmpSimuPrimaryGeneratorAction::SetupSourceType] PayLoadSize   XY:" << PayLoadSizeXY << " Z:" <<PayLoadSizeZ << DmpLogEndl;
  ...
  DmpLogDebug << "[DmpSimuPrimaryGeneratorAction::SetupSourceType] STKSize       XY:" << STKSizeXY     << " Z:" <<STKSizeZ << DmpLogEndl;
  DmpLogDebug << "[DmpSimuPrimaryGeneratorAction::SetupSourceType] STKCenter: " << STKCenter << DmpLogEndl;
  ...
  SetVertexRadius(r0);
  DmpLogDebug << "Generation Sphere radius " << r0 << DmpLogEndl;
  G4ThreeVector dir0; 
  dir0 = G4ThreeVector(0.,0.,1.);
  G4ThreeVector dir1; 
  dir1 = G4ThreeVector(0.,0.,-1.);
  switch(nSourceType) {
  case 0:
  ...
  case 1:
    ...
    //G4cout << "Generation Sphere radius " << posDist->GetRadius() << DmpLogEndl;
  ...
  case 5:
    // Case 5  Generate random position inside the 4PIsphere to create a unif. distrib.-1<Cos(Theta)<-1
    // set position distribution    
    ...
    break;
  case 6:
    // Case 6  Generate random position inside the 4PIsphere to create a unif. distrib.-1<Cos(Theta)<0
    // set position distribution    
    posDist->SetPosDisType("Volume");  //  Point, Plane, Surface or Volume source
    posDist->SetPosDisShape("Sphere");//    Allows the user to choose the particular shape they wish for the     position distribution. Choices are Square, Circle, Ellipse, Rectangle,    Sphere, Ellipsoid, Cylinder, Parallelepiped.
    posDist->SetCentreCoords(G4ThreeVector(0., 0., 0.)); // center of the world
    ...
    break;
  case 7:
    // Case 7  Generate random position inside z<0 half sphere sphere to create a unif. distrib. -1<Cos(Theta)<0
    // set position distribution    
    posDist->SetPosDisType("Volume");  //  Point, Plane, Surface or Volume source
    posDist->SetPosDisShape("Sphere");//    Allows the user to choose the particular shape they wish for the     position distribution. Choices are Square, Circle, Ellipse, Rectangle,    Sphere, Ellipsoid, Cylinder, Parallelepiped.
    posDist->SetCentreCoords(G4ThreeVector(0., 0., 0.)); // center of the world
    ...
    break;
case 8:
  // Case 8 Generate particles in the plane upstream the STK (in between PSD and STK)
  // used for STK Kalman tracking studies ()
  posDist->SetPosDisType("Plane");  //  Point, Plane, Surface or Volume source
  posDist->SetPosDisShape("Rectangle");
  posDist->SetCentreCoords(G4ThreeVector(5*cm, 5*cm, STKCenter - STKSizeZ/2 + 10*micrometer));
  posDist->SetPosRot1(G4ThreeVector(0, 1, 0));
  posDist->SetPosRot2(G4ThreeVector(1, 0, 0));
  angDist->SetAngDistType("iso");
  angDist->SetMinTheta(0.0  * rad);
  angDist->SetMaxTheta(pi/2 * rad);
  angDist->SetMinPhi(0.*rad);
  angDist->SetMaxPhi(twopi *rad);
  posDist->SetHalfY( 0.00001*cm );  // Simply to have all tracks well within the tracker acceptance
...
void DmpSimuPrimaryGeneratorAction::SetupSpectrumType(G4int val)
{
  ...
  //G4cout << "Init nSpectrumType " << nSpectrumType << DmpLogEndl;
...
void DmpSimuPrimaryGeneratorAction::SetRandomSeed(long seed){
...
void DmpSimuPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  case 0:
    ...
    if (dVertexRadius > xy*0.5)
      { 
        DmpLogWarning << "vertexRadius too big " << DmpLogEndl;
        DmpLogWarning << "vertexRadius set to " << xy*0.45 << DmpLogEndl;
	dVertexRadius = xy*0.45;
      }
    
    if (dVertexRadius > z*0.5)
      { 
        DmpLogWarning << "vertexRadius too high " << DmpLogEndl;
        DmpLogWarning << "vertexRadius set to " << z*0.45 << DmpLogEndl;
    ...
    case 2:
      ...
      dir0.setPhi(phi);
      particleGun->SetParticleMomentumDirection((G4ParticleMomentum)dir0);
      //G4cout << " 0000PrimaryGeneratorAction: direction " << diran.x() << " "  << diran.y() << " " << diran.z() << DmpLogEndl;
      particleGun->SetParticlePosition(vertex0);	
      break;      
    case 3:
  ...
  case 1:
    ...
    //G4cout << "loop nSpectrumType " << nSpectrumType << DmpLogEndl;
	...
	z = +(pv_z + pvpart_pz *t);}
      ...
      //G4cout << "b: " <<b <<" c:" << c <<DmpLogEndl;
      //G4cout << "t: "<< t <<" x: " << x<< " y: " << y <<  " z: " << z << " R:" << sqrt(x*x+y*y+z*z)<<DmpLogEndl;
    }
